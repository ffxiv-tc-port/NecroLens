...
public static class DataIds
{
    ...
    //
    // 📌 2026-08-13(第二輪)把死者宮殿與正統優雷卡的區塊補滿。
    //    上游原本各只收錄了區塊裡的一部分(擬態怪 3/5 與 5/10、友方 1/5 與 4/10),
    //    漏掉的那幾個是同一批寶箱/友方模型,只是沒被抄進來 —— 失敗形式是那些個體永遠不被標示,
    //    不會有任何錯誤訊息。本次補進去的 17 個 id 全部逐筆重驗過 ModelChara,零筆不符。
    //    驗證用兩個互相獨立的來源交叉比對,結果一致:
    //      ① `~/.claude/tools/exd/bnpc_id_space.py --range a-b -v`(內建五道校準閘門,含
    //         「BNpcBase 7392 不可以是寶箱模型」這個負向閘門,沒過會 exit 2 且完全不輸出)
    //      ② 直讀 `exd-tc/7.20/BNpcBase.csv`(ModelChara ＝第 4 欄;⚠️ 該 dump 只有一列欄名列,
    //         `rows[1]` 才是列號 0,照「欄名＋型別列」的慣例讀會整批差一)
    //
    ...
    public static readonly HashSet<uint> MimicIDs = new()
    {
        ...
        //   誤植,但不在本次裁決範圍內,保留。
        2566, 6362, 6363,
        // 死者宮殿(2026-08-13 補滿 5831-5835 整個區塊;上游原本只收錄 5832/5834/5835)
        5831, 5832, 5833, 5834, 5835,
        // 天之御柱(2026-08-13 依上述簽名整個區塊收錄)
        9042, 9043, 9044, 9045, 9046, 9047, 9048, 9049, 9050, 9051,
        // 正統優雷卡(2026-08-13 補滿 15996-16005 整個區塊;上游原本只收錄 15997/15998/15999/16002/16003)
    ...
    {
        // 上游既有值,來源不明,本次未動:
        //   5041/7610 的 BNpcBase ModelChara 皆為 0,不屬於友方的 1046 家族,疑似同類 NameId 誤植,
        //   但不在本次裁決範圍內,保留待查。
        5041, 7610,
        // 死者宮殿(2026-08-13 補滿 5836-5840 整個區塊;上游原本只收錄 5840)
        ...
        // 正統優雷卡(2026-08-13 補滿 16006-16015 整個區塊;上游原本只收錄 16007/16008/16009/16012)
